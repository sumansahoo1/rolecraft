...
export function generateLatexSource(spec: ResumeSpec): string {
  ...
  if (contactItems.length > 0) {
    lines.push(`  {\\small\\color{bodytext}${contactItems.join(' \\,$\\\vert$\\, ')}\\par}`);
  }
  ...
  if (spec.projects.length > 0) {
    ...
    for (const proj of spec.projects) {
      lines.push('\\vspace{2pt}');
      lines.push(`{\\bfseries\\color{headline}${escapeLatex(proj.name)}\\par}`);
      lines.push('\\vspace{1pt}');
  ...
  if (spec.education.length > 0) {
    ...
    for (const edu of spec.education) {
      ...
      const leftSide = [
        ...
        edu.institution ? escapeLatex(edu.institution) : '',
      ]
        .filter(Boolean)
        .join(', ');

      lines.push('\\vspace{2pt}');
      lines.push(`{\\color{bodytext}${leftSide}\\hfill{\\small\\color{accent}${rightSide}}\\par}`);
    }
    lines.push('');
  }
...
export { escapeLatex, escapeLatexUrl, formatLinkedinUrl };
