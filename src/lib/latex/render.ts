...
`);
  ...
  if (meta.email) {
    contactItems.push(`<a href="mailto:${escapeHtml(meta.email)}">${escapeHtml(meta.email)}</a>`);
  }
  if (meta.phone) {
  ...
  if (meta.github) {
    const handle = meta.github.replace(/^https?:\/\/(www\.)?github\.com\//, '').replace(/\/$/, '');
    contactItems.push(
  ...
  if (contactItems.length > 0) {
    parts.push(`<div class="contact">${contactItems.join('<span class="sep">|</span>')}</div>`);
  }
  if (meta.targetRole) {
  ...
  if (spec.experience.length > 0) {
    ...
    for (const exp of spec.experience) {
      ...
      parts.push(`<span class="exp-dates">${escapeHtml(exp.dates)}</span>`);
      parts.push('</div>');
  ...
  if (spec.optionalSections && spec.optionalSections.length > 0) {
    for (const opt of spec.optionalSections) {
      parts.push('<div class="section">');
      parts.push(`<div class="section-heading">${escapeHtml(opt.heading)}</div>`);
...
export function renderResumeText(spec: ResumeSpec): string {
  ...
  const contact = [spec.meta.email, spec.meta.phone, spec.meta.linkedin, spec.meta.portfolio]
    .filter(Boolean)
    .join(' | ');
